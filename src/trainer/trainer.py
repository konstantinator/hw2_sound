...
class Trainer(BaseTrainer):
    ...
    def log_predictions(
        ...
        rows = {}
        for ind, (argmax_pred, beam_search_pred, target, raw_pred, audio_path) in enumerate(tuples[:examples_to_log]):
            ...
            self.writer.add_audio(audio=audio_path, audio_name=target, ind=ind, sample_rate=32)
        
        df = pd.DataFrame.from_dict(rows, orient="index")
        self.df = pd.concat((self.df, df))
        self.df = self.df.sort_values(['target', 'epoch'])
        for ind, i in enumerate(self.df.target.unique()):
            self.writer.add_table(
                f"predictions_{ind}", self.df[self.df.target==i]
            )
